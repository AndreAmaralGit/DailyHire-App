...
export default class SearchService extends Component {
      ...
      getData = () => {
            ...
            fetch('http://localhost:8080/workers/findWorkerbyUser', {
                  ...
                  this.setState({ workerID: responseText[0]['_id'], appliedAlert: false, })

                  /************************** GET SERVICES BY WORK AREA *************************/

                  fetch('http://localhost:8080/services/searchServiceWithUser', {
                        method: 'POST',
                        headers: {
                              Accept: 'application/json',
                              'Content-Type': 'application/json',
                        ...
                        for (let i = 0; i < responseText2.length; i++) {
                              ...
                              if (responseText2[i]['user'][0]['_id'] == this.context.user["_id"]) {

                              } else {

                                    /************************** CHECK IF THERE ARE REALTIONS *************************/

                                    fetch('http://localhost:8080/relations/searchRelation', {
                                          method: 'POST',
                                          headers: {
                                                Accept: 'application/json',
                                                'Content-Type': 'application/json',
                                          ...
                                          if (!(responseText3 && responseText3.length > 0)) {
      ...
      applyforService = (serviceID, clientID) => {

            fetch('http://localhost:8080/relations/createRelation', {
                  method: 'POST',
                  headers: {
                        Accept: 'application/json',
                        'Content-Type': 'application/json',
                  ...
                  fetch('http://localhost:8080/notifications/createNotification', {
                        ...
                        this.setState({ appliedAlert: true })
            ...
            return (
                  ...
                  <ScrollView
                        ...
                        <View style={styles.view}>
                              ...
                              {this.state.noServices ? <Text style={{ marginTop: 10 }}>No available services</Text> : null}
                                                      ...
                                                      <Card style={styles.card}>
                                                                  ...
                                                                  <View >
                                                                        ...
                                                                        <View style={{ flexDirection: "row" }}>
                                                                              ...
                                                                              <TouchableOpacity onPress={() => this.setState({ clientModalVisibility: true, clientID: item })}>
                                                                                    <View style={{ flexDirection: "row" }}>
                                                                                          <Avatar.Image size={40} source={{ uri: 'http://192.168.1.69:8080/' + item.user[0].photo }} style={styles.photo} />

                                                                                          <View style={{ marginTop: 12 }}>
                                                                                                <Title style={styles.title}>{item.user[0].name}</Title>
                                                                                          </View>
                                                                                    </View>
                                                                              </TouchableOpacity>
                                                                              ...
                                                                              <IconButton
                                                                                    ...
                                                                                    size={25}
                                                                                    style={styles.open}
                                                                                    onPress={() => this.setState({ messageVisibility: true, receiverID: item.user[0]._id })}
                                                                              />
                                                            ...
                                                            <Card.Actions >
                                                                  ...
                                                                  <View style={{ width: '100%', alignItems: 'flex-end' }}>
                                                                        <Button onPress={() => { this.applyforService(item._id, item.user[0]._id) }}>Apply for the Service</Button>
                                                                  </View>
                                    ...
                                    {this.state.clientModalVisibility ?
                                          ...
                                          <View style={styles.clientModal}>
                                                ...
                                                <View style={{ alignSelf: 'center' }}>
                                                      <Avatar.Image size={65} source={{ uri: 'http://192.168.1.69:8080/' + this.state.clientID.user[0].photo }} style={styles.photo2} />
                                                </View>

                                                <Paragraph style={styles.paragraph}>Name</Paragraph>
                                                <Title style={styles.title}>{this.state.clientID.user[0].name}</Title>

                                                <Paragraph style={styles.paragraph}>Address</Paragraph>
                                                <Title style={styles.title}>{this.state.clientID.user[0].address}</Title>

                                                <Paragraph style={styles.paragraph}>Phone Number</Paragraph>
                                                <Title style={styles.title}>{this.state.clientID.user[0].phoneNumber}</Title>

                                                <Paragraph style={styles.paragraph}>Age</Paragraph>
                                                <Title style={styles.title}>{this.getAge(this.state.clientID.user[0].birthdate)}</Title>

                                                <Paragraph style={styles.paragraph}>Gender</Paragraph>
                                                <Title style={styles.title}>{this.state.clientID.user[0].gender}</Title>
                                                ...
                                                <View style={{ flexDirection: "row" }}>
                                                      <Rating
                                                            ...
                                                            startingValue={this.getRating(this.state.clientID.user[0].ratingCounter, this.state.clientID.user[0].ratingTotal)}
                                                      />
                                                      <Title style={styles.ratingNumber}>{this.getRatingText(this.state.clientID.user[0].ratingCounter, this.state.clientID.user[0].ratingTotal)}</Title>
                                                </View>
